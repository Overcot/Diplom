function [ x ] = Boundary(x0, u)
    ...
    for time=t(1:end - 1)
        ...
        x(s(end), time + 1) = Dt*(-mu(s(end)-1) * x(s(end)-1, time) - u(s(end)-1, time)*(1 - mu(s(end)-1)) * x(s(end)-1, time) + x(s(end)-1, time)/Dt) + Dt*(-mu(s(end)) * x(s(end), time) - u(s(end), time)*(1 - mu(s(end))) * x(s(end), time) + x(s(end), time)/Dt);
    end
