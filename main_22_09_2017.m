...
while (lambda1 <= 1)
    ...
    while (k < 100000) && (prev > 10e-3)
        ...
        beta = k*sqrt(Ds^2+Dt^2)/norm(J2_);
        if beta < 0.1
            beta = 0.1;
        end
    ...
    plotGraph(CorrectX(end,:),xu(end,:), {0:T}, 't', 'x(s=7, t)');
    plotGraph(CorrectX(:,t(end)),xu:end,t(end)), {0:L}, 's', 'x(s, t=8)');
    plotGraph(CorrectX(:,1*T_steps/T+1),xu(:,1*T_steps/T+1), {0:L}, 's', 'x(s, t=1)');
    plotGraph(CorrectX(:,2*T_steps/T+1),xu(:,2*T_steps/T+1), {0:L}, 's', 'x(s, t=2)');
    plotGraph(CorrectX(1,:),xu(1,:), {0:T}, 't', 'x(s=0, t)');
    plotGraph(CorrectU(end,:),uk(end,:), {0:T}, 't', 'u(s=7, t)');
    plotGraph(CorrectU(:,t(end)),uk(:,t(end)), {0:L}, 's', 'u(s, t=8)');
    plotGraph(CorrectU(:,1*T_steps/T+1),uk(:,1*T_steps/T+1), {0:L}, 's', 'u(s, t=1)');
    plotGraph(CorrectU(:,2*T_steps/T+1),uk(:,2*T_steps/T+1), {0:L}, 's', 'u(s, t=2)');
    plotGraph(CorrectU(1,:),uk(1,:), {0:T}, 't', 'u(s=0, t)');
