...
gamma(3) = 1/4; % для примера чтобы выровнять нач точку
x(s(1:end), 1) = 1;
mu(s(1:end)) = (s-1)/L*Ds;
u(s(1:end),t(1:end)) = 0;
...
for time=t(1:end - 1)
    ...
    %считаем самую левую точку из краевого условия в задаче
    x(1,time+1) = trapz(gamma(1:end)'.*x(1:end, time)*Ds);
    %считаем самую правую точку с помощью явного левого уголка
