...
xu = Boundary(CorrectX(s,1), CorrectU);
    ...
    plotGraph(CorrectX(2*S_steps/L+1,:), xu(2*S_steps/L+1, :), {0:T}, 't', 'x(s=2, t)');
    plotGraph(CorrectX(5*S_steps/L+1,:), xu(5*S_steps/L+1, :), {0:T}, 't', 'x(s=5, t)');
    plotGraph(CorrectX(end,:), xu(end, :), {0:T}, 't', 'x(s=7, t)');
    plotGraph(CorrectX(:,1), xu(:,1), {0:L}, 's', 'x(s, t=0)');
    plotGraph(CorrectX(:,2*T_steps/T+1), xu(:, 2*T_steps/T+1), {0:L}, 's', 'x(s, t=2)');
    plotGraph(CorrectX(:,5*T_steps/T+1), xu(:, 5*T_steps/T+1), {0:L}, 's', 'x(s, t=5)');
    plotGraph(CorrectX(:,end), xu(:,end), {0:L}, 's', 'x(s, t=8)');